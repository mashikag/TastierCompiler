...
/*--------------------------------------------------------------------------*/
  ...
  public void Write (string filename) {
	...
	void VarDecl(bool external) {
		Symbol sym; string name; TastierType type; Scope currentScope = openScopes.Peek();
		...
		if (la.kind == 6) {
			Get();
			Expect(2);
			int arrayLength = Convert.ToInt32(t.val);
			program.Add(new Instruction("","Const " + arrayLength));
			...
			for(int i = 0; i < arrayLength; i++){
			 ...
			 Symbol elemSym;
			 if (external) {
			   elemSym = new Symbol(name+"["+i+"]", (int)TastierKind.Var, (int)type, 0, 0);
			   externalDeclarations.Push(elemSym);
			 } else {
			   elemSym = new Symbol(name+"["+i+"]", (int)TastierKind.Var, (int)type, openScopes.Count-1, currentScope.Count( s => s.Item2 == (int)TastierKind.Var ));
	...
	static readonly bool[,] set = {
		...
		{x,T,x,x, x,x,x,x, x,x,x,x, T,x,x,T, x,x,x,x, x,x,x,x, x,x,x,T, x,x,T,T, x,T,T,T, T,x,T,T, T,T,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,x,x, x,x,x,x},
		{x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,x,x,x, x,x,x,T, x,x,T,T, x,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x},
		{x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,x,x,x, x,x,x,T, x,x,T,T, x,T,T,T, T,x,T,T, T,T,x,x, x,x,x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,T, T,T,x,x, T,x,x,x}
...
public class Errors {
	...
	public virtual void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 54: s = "invalid ConstDecl"; break;
